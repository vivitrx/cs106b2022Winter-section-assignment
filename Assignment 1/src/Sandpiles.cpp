...
void dropSandOn(Grid<int>& world, int row, int col) {
    ...
    if (world.inBounds(row, col) == true && world[row][col] <= 3) // base case 如果这个格子上存储的数字小于等于3，那么直接++
    {
        world[row][col] += 1;
    }else
    {
    ...
    if (world[row][col] > 3) // 如果大于3，那么对他周围的4个格子执行dropSandOn()
    {
        world[row][col] = 0;

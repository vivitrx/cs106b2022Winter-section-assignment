...
RobinHoodHashTable::RobinHoodHashTable(HashFunction<string> hashFn) {
    ...
    for (int i = 0; i < hashFn.numSlots(); i++)
    {
        ...
        elems[i].value = "57+g5pif55+z"; // 初始化为一个复杂值
    }
...
bool RobinHoodHashTable::insert(const string& elem) {
    ...
    if (contains(elem)){
        cout << "please not insert duplicates" << endl;
        return false;
    }

    Slot main_opreator; // 主要操作对象，可能会改变
    main_opreator.value = elem;
    main_opreator.distance = 0;
    // 现在插入一个节点
    int hash_code = HashFFFFn(elem); // 先计算出elem的hash code;
    // 情况1：如果一切都很顺利，hashcode指示的地点是空槽
    if (elems[hash_code].distance == EMPTY_SLOT){ // 直接装入
        elems[hash_code].value = main_opreator.value;
        elems[hash_code].distance = 0;
        // printDebugInfo();
        actual_size++;
        return true;
    }
    // 情况2：如果不那么顺利，hashcode指示的地方不是空槽，那么对比DSL
    if (elems[hash_code].distance != EMPTY_SLOT){
        int i = hash_code;
        while(elems[i].distance != EMPTY_SLOT){
            if (main_opreator.distance <= elems[i].distance){
                // 如果PSL小于等于当地人的PSL，那就move on
                main_opreator.distance++;
                i++;
                if (i == array_size) { // 到达数组边界，返回到数组头部
                    i = 0;
                }
            }else{ // 进行交换
                Slot temp = elems[i];
                elems[i].distance = main_opreator.distance; elems[i].value = main_opreator.value;
                main_opreator = temp;
        ...
        // 如果找到了一个空槽
        elems[i].value = main_opreator.value;
        elems[i].distance = main_opreator.distance;
    }
    // printDebugInfo();
    actual_size++;
    return true;
...
bool RobinHoodHashTable::contains(const string& elem) const {
    ...
    while(true)
    {
        ...
        if (distance > elems[i].distance){ // 遍历过程中，所寻找元素当前的distance > 当前查看元素的distance
            return false;
        }
        i++;
        if(i == array_size){ // 如果越界，应该返回到开头
            i = 0;
        }
        distance++;
    }
...
bool RobinHoodHashTable::remove(const string& elem) {
    ...
    // 先计算hashcode
    int i = HashFFFFn(elem);
    while(elems[i].value != elem){
        // 如果这个slot存储的value不是我们要找的value，就得继续往下找
        i++;
        if (i == array_size) { // 到达数组边界，返回到数组头部
            i = 0;
        }
    }
    // if (elems[i].value == elem){ // 如果一切顺利
    elems[i].distance = EMPTY_SLOT;
    actual_size--;
    // 然后开始shift back
    Shiftback(i);
...
void RobinHoodHashTable::Shiftback(int i){
    // shift back直到遇到一个PSL为0的slot
    while(true){
        i++;
        if (i == array_size) { // 到达数组边界，返回到数组头部
            i = 0;
            if(elems[i].distance == 0 || elems[i].distance == EMPTY_SLOT){
                elems[array_size - 1].distance = EMPTY_SLOT;
                break;
            }
            // 将0号位后移到最后一号
            elems[array_size - 1].value = elems[i].value;
            elems[array_size - 1].distance = elems[i].distance;
            elems[array_size - 1].distance--;
            continue;
        }

        if(elems[i].distance == 0 || elems[i].distance == EMPTY_SLOT){
            elems[i-1].distance = EMPTY_SLOT;
            break;
        }
        elems[i-1].value = elems[i].value;
        elems[i-1].distance = elems[i].distance;
        elems[i-1].distance--;
...
bool RobinHoodHashTable::isFull(){

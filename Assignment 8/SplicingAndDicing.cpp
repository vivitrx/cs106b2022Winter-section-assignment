...
string fromDNA(Nucleotide* dna) {
    string result = "";
    Nucleotide* current_dna = dna;
    while(current_dna != nullptr){
        result += current_dna->value;
...
Nucleotide* toStrand(const string& str) {
    ...
    new_dna->value = str[0]; new_dna->prev = nullptr; new_dna->next = nullptr;
    Nucleotide* RESULT = new_dna;
    Nucleotide* prev_dna = new_dna;

    for(int i = 1; i != length; i++){
        // 制作new_dna与前一个dna的双向链表;
        new_dna = new Nucleotide;
        prev_dna->next = new_dna;
        new_dna->value = str[i]; new_dna->prev = prev_dna; new_dna->next = nullptr;
        prev_dna = new_dna;
    }
    return RESULT;
}
...
Nucleotide* findFirst(Nucleotide* dna, Nucleotide* target) {
    ...
    Nucleotide* tg_down = target; Nucleotide* RESULT = nullptr;
    Nucleotide* dna_up = dna;
    Nucleotide* incremental_ptr = dna;
    int findSuccessful = 0;
    while(dna_up != nullptr){
        // 遍历搜索dna链，寻找与tg_down->value相同的dna_up->value;
        if (tg_down->value == dna_up->value){
            if (findSuccessful == 0){
                RESULT = dna_up;
            }
            tg_down = tg_down->next;
            if (tg_down == nullptr){
                break;
            }
            dna_up = dna_up->next;
            ...
            // 回去再试试，如果还是不行再前进
            if (tg_down->value == dna_up->value){
                dna_up = incremental_ptr->next;
                incremental_ptr = incremental_ptr->next;
                continue;
            }else{
                dna_up = dna_up->next;
            }
    ...
    // 当他遍历完整个dna链却仍然没有找到target的时候，这就是失败
    if (dna_up == nullptr && tg_down != nullptr){
        return nullptr;
    }
    // 当他在遍历完或者遍历完dna链之前，找到target，这就是成功
    else { //if (dna_up != nullptr && tg_down == nullptr)
        return RESULT;
    }
}
...
bool spliceFirst(Nucleotide*& dna, Nucleotide* target) {
    ...
    // 先使用findFirst(dna, target)找到target的位置;
    Nucleotide* location = findFirst(dna, target);
    Nucleotide* target_copy = target;
    Nucleotide* EndNode = location; // 寻找尾节点
    Nucleotide* junk = location;
    if (location == nullptr){
        return false; // target并不存在于dna中
    }
    while (target_copy->next != nullptr){ // 寻找尾节点EndNode
        EndNode = EndNode->next;
        target_copy = target_copy->next;
    }
    target_copy = target;
    if (location->prev == nullptr){ // 对应要删的链的头节点在开头的情况
        // 直接删除即可，这种情况不需要拼接;
        while (target_copy != nullptr){ // 遍历删除
            dna = dna->next;
        ...
        return true;
    }else if (EndNode->next == nullptr){ // 要删的链的尾节点在结尾的情况
        // 直接删除即可，这种情况不需要拼接;
        Nucleotide* location_prev = location->prev;
        while (target_copy != nullptr){ // 遍历删除
            location = location->next;
            delete junk;
            junk = location;
        ...
        location_prev->next = nullptr;
        return true;
    }else{ // 最寻常的情况，要删的链在中间
        location->prev->next = EndNode->next; // 拼接
        EndNode->next->prev = location->prev;
        while (target_copy != nullptr){ // 遍历删除
            location = location->next;
            delete junk;
            junk = location;
...
// STUDENT_TEST("spliceFirst STUDENT TEST.") {
//         Nucleotide* dna    = toStrand("ACGTAC");
//         Nucleotide* target = toStrand("ACG");

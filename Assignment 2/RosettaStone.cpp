...
Map<string, double> kGramsIn(const string &text, int kGramLength) {
     ...
     * this function.
     * 我觉得最好用递归来做，因为传值传递可以回避掉循环中“把string中拿出来的值放回去”的问题
     *
     * 不如循环
     */
    Map<string, double> Gram;
    // 检测输入合法性
    if (kGramLength <= 0)
    {
        error("哦哥们，别输入零或负数啊");
    ...
    if (kGramLength > text.length()) // 如果 k-gram 长度大于输入字符串的长度，则应返回一个空的 Map
    {
        return Gram;
    }

    int length = text.length() - kGramLength + 1;
    for (int i = 0; i < length; i++)
    {
        string simple = text.substr(i, kGramLength);

        if (Gram.get(simple) >= 1) // 往map里装填kGram
        { // 如果key存在于Map中，增加该 key 的计数。
            double temp = Gram.get(simple);
            temp += 1;
            Gram.put(simple, temp);
        }else // 如果key尚不存在于Map中，则初始化该 key 的计数为 1。
        {
            Gram.put(simple, 1);
        }
    }
...
Map<string, double> normalize(const Map<string, double>& input) {
    ...
    for (string key : input) {
        frequencies += input[key] * input[key];
    }
    if (frequencies == 0)
    {
        error("请勿传入value全是0的Map");
    }

    Map<string, double> normalize;
    for (string key : input) {
        normalize.put(key, input[key] / sqrt(frequencies));
    }

    return normalize;
}

// 2024年5月24日01:06:54：已完成

Map<string, double> topKGramsIn(const Map<string, double>& profile, int numToKeep) {
    /* TODO: Delete this comment and the other lines here, then implement
     * this function.
     */
    if (numToKeep < 0){
        error("do not enter negative!");
    }
    if (numToKeep > profile.size()){
        return profile;
    }
    if (numToKeep == 0){
        return {};
    ...
    // 然后把profile中的所有entities压入PriorityQueue中
    for (string key : profile)
    {
        PQueue.enqueue(key, profile[key]);
    }

    Map<string, double> target;
    int time = PQueue.size() - numToKeep;
    for (int i = 0; i < time; i++) //弹出没有用的entity
    {
        PQueue.dequeue();
    }
    ...
    {
        double temp = PQueue.peekPriority();
        target.put(PQueue.dequeue(), temp);
    }
    return target;
}

// 2024年5月24日12:48:01：完成
...
double cosineSimilarityOf(const Map<string, double>& lhs, const Map<string, double>& rhs) {
    ...
    for (string key1 : lhs)
    {
        for (string key2 : rhs)
        {
            if (key1 == key2)
            {
                cosine_similarity += lhs[key1] * rhs[key2];
            }
        }
    }
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    {
        temp_rank = cosineSimilarityOf(textProfile, corpus.profile);
        if (temp_rank > rank)
        {
            rank = temp_rank;
            rank_name = corpus.name;
        }
    }
    return rank_name;
}
...
PROVIDED_TEST("请关注错误测试！kGramsIn reports errors on bad inputs.") {
...
PROVIDED_TEST("Normalize不添加或删除键。normalize does not add or remove keys.") {

...
#include "set.h"
using namespace std;
string pangrammaticString(); // 借我用一下
/**
 ...
 */
EncodingTreeNode* helper(EncodingTreeNode*& left,
...
EncodingTreeNode* huffmanTreeFor(const string& str) {
    ...
    for(char word : map){
        ...
        node->one = nullptr;
        node->zero = nullptr;
        pq.enqueue(node, map[word]);
    }

    EncodingTreeNode* left = pq.dequeue(); // 0号节点
    EncodingTreeNode* right = pq.dequeue(); // 1号节点
    return helper(left, right, pq, map);
}

EncodingTreeNode* helper(EncodingTreeNode*& left, // 0号节点
                         EncodingTreeNode*& right, // 1号节点
                         PriorityQueue<EncodingTreeNode*> pq,
                         Map<char, double> map){
    // base case
    ...
    if(pq.isEmpty()){
        ...
        map.put(newNode->ch, map[left->ch] + map[right->ch]);
        pq.enqueue(newNode, map[newNode->ch]);
        left = pq.dequeue();
        right = pq.dequeue();
        return helper(left, right, pq, map);
    }
 ...
 */
string decodeText_helper(Queue<Bit>& bits,
                  EncodingTreeNode* tree,
                  string RESULT,
                  EncodingTreeNode* original_node);
string decodeText(Queue<Bit>& bits, EncodingTreeNode* tree) {
    string RESULT = "";
    string intermedia = "";
    EncodingTreeNode* original_node = tree;
    while(!bits.isEmpty()){
        RESULT += decodeText_helper(bits, tree, intermedia, original_node); // 一次返回一个字母
    }
    // return decodeText_helper(bits, tree, RESULT, original_node);
    return RESULT;
}
string decodeText_helper(Queue<Bit>& bits,
                         EncodingTreeNode* tree,
                         string RESULT,
                         EncodingTreeNode* original_node){
    if(tree->one == nullptr || tree->zero == nullptr/*bits.isEmpty()*/){ // base case
        return RESULT;
    }else{ // recursive case
        Bit bbbit = bits.dequeue();
        if (bbbit == 0){ // 左移
            tree = tree->zero;
            if(tree->one == nullptr || tree->zero == nullptr){ // 关键在于“他是叶子节点”
                RESULT += tree->ch;
                return decodeText_helper(bits, tree, RESULT, original_node);
            }else{
                return decodeText_helper(bits, tree, RESULT, original_node);
 ...
 */
void Map_maker(Map<char,vector<Bit>>& map,
               EncodingTreeNode* tree,
               vector<Bit> bbbits,
               char balance);
Queue<Bit> encodeText(const string& str, EncodingTreeNode* tree) {
    ...
    vector<Bit> bbbits;
    Map_maker(EncodingMap, tree, bbbits, 'N');
    Queue<Bit> RESULT;
    for(int i = 0; i < str.size(); i++){
        for(int j = 0; j < EncodingMap[str[i]].size(); j++){
            if(EncodingMap[str[i]][j] == 0){
                RESULT.enqueue(0);
            }else{
                RESULT.enqueue(1);
            }
...
void Map_maker(Map<char, vector<Bit>>& map,
    ...
    if(tree->zero != nullptr){
        if(balance == '0'){
            Bit zero = 0;
            bbbits.push_back(zero);
        }else if(balance == '1'){
            Bit one = 1;
            bbbits.push_back(one);
        }
        // map[tree->ch] = bbbits;
        Map_maker(map, tree->zero, bbbits, '0');
        Map_maker(map, tree->one, bbbits, '1');
    }else{
        if(balance == '0'){
            Bit zero = 0;
            bbbits.push_back(zero);
 ...
 * or bits in them, etc.
 */
void decodeTree_helper(EncodingTreeNode* tree,
                       Queue<Bit>& bits,
                       Queue<char>& leaves,
                       EncodingTreeNode*& root // 仅作debug用
                       );
EncodingTreeNode* decodeTree(Queue<Bit>& bits, Queue<char>& leaves) {
    ...
    decodeTree_helper(tree, bits, leaves, root);
    return root;
}
void decodeTree_helper(EncodingTreeNode* tree,
                       Queue<Bit>& bits,
    ...
    if (bits.isEmpty()) { // base case
        return;
    }else{
        if(bits.dequeue() == 1){
            tree->zero = new EncodingTreeNode;
            decodeTree_helper(tree->zero, bits, leaves, root);
            tree->one = new EncodingTreeNode;
            decodeTree_helper(tree->one, bits, leaves, root);
        }else{ // bits.dequeue() == 0
            tree->ch = leaves.dequeue();
            tree->zero = nullptr;
            tree->one = nullptr;
            // 然后他就会自动返回上一个call stack
        }
    }
}
...
HuffmanResult compress(const string& text) {
    // 如果要压缩的输入字符串中没有至少两个不同的字符，您的实现就应该调用 error()。
    Set<char> sssset;
 ...
 */
/* 测试一个被认为是 Huffman 树的树是否确实为 Huffman 树的辅助函数。
 * 具体来说，这里检查每个内部节点是否要么没有子节点，要么有两个子节点。
 * 还有其他方法可以构造一个无效的 Huffman 树——例如，包含未初始化的指针或形成循环链接——
 * 但我们在这里不测试这些情况。
 */
